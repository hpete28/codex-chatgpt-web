...
const en = {
  ...
  biggerContextBody: "Warning: keeps small turns as one message, then stages large context across two to eight messages as needed before a small execution request. Extra transport messages do not expand the three-times context ceiling. It triples the model context and compaction thresholds; restart Codex after changing it. Extra requests may increase rate limits or temporary cooldowns. Disabled by default.",
  ...
  observedTurn: "Observed turn",
  observedPhase: "Phase",
  lastObservedProgress: "Last observed progress",
  multipartAcknowledgement: "Multipart ACK",
  continuationCount: "Continuations",
  transportOutcome: "Transport outcome",
  modelReportedWorkState: "Model-reported work state",
  noTurnObservations: "No turn observations available.",
  unavailable: "Unavailable",
  recentActivity: "Recent events",
  noLogs: "No runtime events yet.",
  exportSafeLog: "Export safe log",
  exportDiagnosticReport: "Export diagnostic report",
  ...
  sourceCondition: "Source state",
  cleanBuild: "Clean",
  dirtyBuild: "Dirty",
  unknownBuild: "Unknown",
  runtimeBundleId: "Runtime bundle ID",
  buildMismatch: "Launcher and runtime build identities do not match. Install a reviewed, matching bundle before running Codex tasks.",
  customUpdaterDisabled: "Public updater is disabled for custom builds. Updates use reviewed upstream integration and a separately authorized installation.",
  unknownUpdaterDisabled: "Public updater is disabled because this build identity is unknown. Verify the installation source before installing an update.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：较小回合仍作为一条消息发送；较大的上下文会根据需要分成两到八条暂存消息，然后发送一个较小的执行请求。更多传输消息不会扩大三倍语义上下文上限。模型上下文和压缩阈值将扩大三倍；更改后请重启 Codex。额外请求可能提高触发速率限制或临时冷却的概率。默认关闭。",
  ...
  observedTurn: "已观测回合",
  observedPhase: "阶段",
  lastObservedProgress: "最近观测进度",
  multipartAcknowledgement: "多段 ACK",
  continuationCount: "继续次数",
  transportOutcome: "传输结果",
  modelReportedWorkState: "模型报告的工作状态",
  noTurnObservations: "暂无回合观测数据。",
  unavailable: "不可用",
  recentActivity: "最近事件",
  noLogs: "暂无运行时事件。",
  ...
  sourceCondition: "源代码状态",
  cleanBuild: "干净",
  dirtyBuild: "有未提交更改",
  unknownBuild: "未知",
  runtimeBundleId: "运行时包 ID",
  buildMismatch: "启动器与运行时的构建标识不匹配。在运行 Codex 任务前，请安装经过审核且匹配的构建包。",
  customUpdaterDisabled: "此自定义构建已禁用公共更新器。更新需经过审核的上游集成，并在单独授权后安装。",
  unknownUpdaterDisabled: "由于无法识别此构建，公共更新器已禁用。安装更新前请先验证安装来源。",
  launchAtLogin: "登录时启动",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小さなターンは 1 件のメッセージとして維持し、大きなコンテキストは必要に応じて 2〜8 件のステージングメッセージに分割し、その後に小さな実行リクエストを送信します。追加の転送メッセージで 3 倍のセマンティックコンテキスト上限が広がることはありません。モデルのコンテキスト上限とコンパクションしきい値は 3 倍になります。変更後は Codex を再起動してください。追加リクエストにより、レート制限や一時的なクールダウンが発生しやすくなる場合があります。初期設定では無効です。",
  ...
  observedTurn: "観測ターン",
  observedPhase: "フェーズ",
  lastObservedProgress: "最後に観測した進行",
  multipartAcknowledgement: "マルチパート ACK",
  continuationCount: "継続回数",
  transportOutcome: "転送結果",
  modelReportedWorkState: "モデル報告の作業状態",
  noTurnObservations: "ターン観測データはありません。",
  unavailable: "利用不可",
  recentActivity: "最近のイベント",
  noLogs: "ランタイムイベントはまだありません。",
  exportSafeLog: "安全なログをエクスポート",
  exportDiagnosticReport: "診断レポートをエクスポート",
  settingsTitle: "ランチャー設定",
  general: "一般",
  buildIdentity: "ビルド情報",
  launcherBuild: "ランチャービルド",
  runtimeBuild: "ランタイムビルド",
  distribution: "配布種別",
  sourceRevision: "ソースリビジョン",
  sourceCondition: "ソースの状態",
  cleanBuild: "クリーン",
  dirtyBuild: "変更あり",
  unknownBuild: "不明",
  runtimeBundleId: "ランタイムバンドル ID",
  buildMismatch: "ランチャーとランタイムのビルド情報が一致しません。Codex タスクを実行する前に、レビュー済みの一致するバンドルをインストールしてください。",
  customUpdaterDisabled: "カスタムビルドでは公開アップデーターが無効です。更新には、レビュー済みのアップストリーム統合と、別途承認されたインストールを使用します。",
  unknownUpdaterDisabled: "このビルド情報を確認できないため、公開アップデーターは無効です。更新をインストールする前に、インストール元を確認してください。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "경고: 작은 턴은 하나의 메시지로 유지하고, 큰 컨텍스트는 필요에 따라 2~8개의 스테이징 메시지로 나눈 뒤 작은 실행 요청을 보냅니다. 추가 전송 메시지는 3배의 의미적 컨텍스트 한도를 늘리지 않습니다. 모델 컨텍스트와 컴팩션 임계값은 3배로 증가합니다. 변경 후 Codex를 다시 시작하세요. 추가 요청으로 인해 사용량 제한이나 일시적인 쿨다운이 더 자주 발생할 수 있습니다. 기본적으로 비활성화되어 있습니다.",
  ...
  observedTurn: "관찰된 턴",
  observedPhase: "단계",
  lastObservedProgress: "마지막 관찰 진행",
  multipartAcknowledgement: "멀티파트 ACK",
  continuationCount: "계속 횟수",
  transportOutcome: "전송 결과",
  modelReportedWorkState: "모델 보고 작업 상태",
  noTurnObservations: "턴 관찰 데이터가 없습니다.",
  unavailable: "사용할 수 없음",
  recentActivity: "최근 이벤트",
  noLogs: "아직 런타임 이벤트가 없습니다.",
  exportSafeLog: "안전한 로그 내보내기",
  exportDiagnosticReport: "진단 보고서 내보내기",
  settingsTitle: "런처 설정",
  general: "일반",
  buildIdentity: "빌드 ID",
  launcherBuild: "런처 빌드",
  ...
  sourceCondition: "소스 상태",
  cleanBuild: "정상",
  dirtyBuild: "변경 사항 있음",
  unknownBuild: "알 수 없음",
  runtimeBundleId: "런타임 번들 ID",
  buildMismatch: "런처와 런타임 빌드 ID가 일치하지 않습니다. Codex 작업을 실행하기 전에 검토를 거친 일치하는 번들을 설치하세요.",
  customUpdaterDisabled: "사용자 지정 빌드에서는 공개 업데이터가 비활성화됩니다. 업데이트에는 검토를 거친 업스트림 통합과 별도로 승인된 설치를 사용합니다.",
  unknownUpdaterDisabled: "이 빌드 ID를 확인할 수 없어 공개 업데이터가 비활성화됩니다. 업데이트를 설치하기 전에 설치 출처를 확인하세요.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小型回合仍以一則訊息傳送；大型上下文會視需要拆成兩到八則暫存訊息，之後再傳送一個小型執行請求。額外的傳輸訊息不會擴大三倍語意上下文上限。模型上下文與壓縮門檻會提高至三倍；變更後請重新啟動 Codex。額外請求可能更容易觸發速率限制或暫時冷卻。預設關閉。",
  ...
  observedTurn: "已觀測回合",
  observedPhase: "階段",
  lastObservedProgress: "最近觀測進度",
  multipartAcknowledgement: "多段 ACK",
  continuationCount: "繼續次數",
  transportOutcome: "傳輸結果",
  modelReportedWorkState: "模型回報的工作狀態",
  noTurnObservations: "目前沒有回合觀測資料。",
  unavailable: "無法取得",
  recentActivity: "最近事件",
  noLogs: "尚無執行階段事件。",
  exportSafeLog: "匯出安全日誌",
  exportDiagnosticReport: "\u532f\u51fa\u8a3a\u65b7\u5831\u544a",
  settingsTitle: "啟動器設定",
  general: "一般",
  buildIdentity: "建置識別",
  launcherBuild: "啟動器建置",
  ...
  sourceCondition: "原始碼狀態",
  cleanBuild: "乾淨",
  dirtyBuild: "有未提交變更",
  unknownBuild: "未知",
  runtimeBundleId: "執行階段套件 ID",
  buildMismatch: "啟動器與執行階段的建置識別不一致。執行 Codex 任務前，請安裝經過審查且相符的套件。",
  customUpdaterDisabled: "此自訂建置已停用公開更新程式。更新需經過審查的上游整合，並在另行授權後安裝。",
  unknownUpdaterDisabled: "由於無法識別此建置，公開更新程式已停用。安裝更新前請先驗證安裝來源。",
  launchAtLogin: "登入時啟動",
